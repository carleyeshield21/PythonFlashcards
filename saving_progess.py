...
# we will now change from where the function originally reads the file at the start of the program, but this will create a FileNotFound error the first time it runs since it has not yet created a file, we will use handling exceptions
# we also need to set a global variable to_learn
...
# this function will remove the known words from the data which is chosen by the user by clicking on the check button
# we will use pandas in this function to create a new dataframe, and save it as a csv file, and save it to the desired folder in this case is the data folder
def is_known():
